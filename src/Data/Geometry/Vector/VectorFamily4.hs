...
--   Vector :: Vec d r => VectorF d r -> Vector d r



newtype Func a b   (s :: SelectD) = Func { runFunc :: a s -> b s }
newtype VecF d a b (s :: SelectD) = VecF { runVF' :: VectorFamily s d a -> VectorFamily s d b}
...
instance Arity d => Functor (Vector d) where
  fmap f = Vector . gmap (Proxy :: Proxy (SelectF d)) (Proxy :: Proxy d) f . _unV

gmap       :: (VectorSelect s, V.Arity d)
           => proxy s -> prox d -> (a -> b) -> VectorFamily s d a -> VectorFamily s d b
gmap s d f = runVF d $ select s (VecF $ fmap f)
                                (VecF $ fmap f)
                                (VecF $ fmap f)
                                (VecF $ fmap f)
                                (VecF $ fmap f)
...
-- _unV             :: Vector d r -> VectorF d r
-- _unV ~(Vector v) = v

-- unV :: forall d r s. Lens (Vector d r) (Vector d s) (VectorF d r) (VectorF d s)
-- unV = lens _unV mkVector
--   where
--     mkVector              :: Vector d r -> VectorF d s -> Vector d s
--     mkVector (Vector _) v = Vector v

-- type instance V.Dim (Const ()) = 0
-- type instance V.Dim Identity = 1
-- type instance V.Dim L2.V2 = 2
-- type instance V.Dim L3.V3 = 2
-- type instance V.Dim L4.V4 = 2
...
-- -- deriving instance (Functor (VectorF d))      => Functor (Vector d)
-- -- deriving instance (Foldable (VectorF d))     => Foldable (Vector d)
-- -- deriving instance (Traversable (VectorF d))  => Traversable (Vector d)
-- -- deriving instance (Applicative (VectorF d))  => Applicative (Vector d)

-- -- deriving instance (Eq (VectorF d r), Eq r)   => Eq (Vector d r)
-- -- deriving instance (Ord (VectorF d r), Ord r) => Ord (Vector d r)

-- -- instance (Foldable (VectorF d), Show r) => Show (Vector d r) where
-- --   show v = mconcat [ "Vector", show $ F.length v , " "
-- --                    , show $ F.toList v ]

-- -- deriving instance (Additive (VectorF d)) => Additive (Vector d)

-- -- -- instance Arity d => Additive (Vector d) where
-- -- --   zero = pure 0
-- -- --   (Vector u) ^+^ (Vector v) = Vector $ V.zipWith (+) u v

-- -- instance Additive (VectorF d) => Affine (Vector d) where
-- --   type Diff (Vector d) = Vector d
-- --   u .-. v = u ^-^ v
-- --   p .+^ v = p ^+^ v

-- -- deriving instance Metric (VectorF d) => Metric (Vector d)
